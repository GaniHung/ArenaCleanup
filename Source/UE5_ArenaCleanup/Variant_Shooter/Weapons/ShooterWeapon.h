 ...
 */
UCLASS(abstract)
class UE5_ARENACLEANUP_API AShooterWeapon : public AActor
{
	GENERATED_BODY()
...
protected:
	...
	/** Spawn scale for fired projectiles */
	UPROPERTY(EditAnywhere, Category="Ammo")
	FVector ProjectileScale = FVector(0.25f);
	...
	/** Total bullets available outside the current magazine */
	UPROPERTY(EditAnywhere, Category="Ammo", meta = (ClampMin = 0, ClampMax = 999))
	int32 ReserveAmmo = 30;

	/** If true, this weapon does not consume ammo and never reloads */
	UPROPERTY(EditAnywhere, Category="Ammo")
	...
	/** Time before a reload transfers reserve ammo into the magazine */
...
public:
	...
	/** Reload this weapon from reserve ammo */
	void Reload();
...
protected:
	...
	/** Returns true if this weapon can fire a shot now */
	bool CanFire() const;

	/** Returns true if this weapon can reload now */
	bool CanReload() const;

	/** Completes a pending reload */
	void FinishReload();
...
public:
	...
	/** Returns the reserve ammo count */
	int32 GetReserveAmmo() const { return bInfiniteAmmo ? -1 : ReserveAmmo; }

	/** Refills this weapon when the player picks up the same weapon again */
	void RefillAmmo();

	/** Returns true when this weapon has unlimited ammo */
	bool HasInfiniteAmmo() const { return bInfiniteAmmo; }

	/** Sets whether this weapon should ignore ammo limits */
	void SetInfiniteAmmo(bool bInInfiniteAmmo) { bInfiniteAmmo = bInInfiniteAmmo; }
};
