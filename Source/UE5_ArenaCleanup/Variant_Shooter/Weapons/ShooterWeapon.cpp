...
{
	if (!GetOwner() || !GetOwner()->HasAuthority() || !CanReload())
	{
		return;
	}
...
{
	return !bInfiniteAmmo && !bIsReloading && CurrentBullets < MagazineSize && ReserveAmmo > 0;
}
...
void AShooterWeapon::FinishReload()
{
	...
	const int32 BulletsToLoad = FMath::Min(MissingBullets, ReserveAmmo);
...
{
	if (!GetOwner() || !GetOwner()->HasAuthority() || bInfiniteAmmo)
	{
		return;
	}
	...
	bIsReloading = false;
	CurrentBullets = MagazineSize;
	ReserveAmmo = FMath::Max(ReserveAmmo, 30);
	WeaponOwner->UpdateWeaponHUD(CurrentBullets, MagazineSize, GetReserveAmmo());
}
...
FTransform AShooterWeapon::CalculateProjectileSpawnTransform(const FVector& TargetLocation) const
{
	...
	return FTransform(AimRot, SpawnLoc, ProjectileScale);
}
